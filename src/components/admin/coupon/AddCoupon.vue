  ...
  <form @submit.prevent="handleSubmit" class="text-left font-sans flex flex-col">
    ...
    <div class="p-6 bg-[#132212] text-white flex items-start justify-between border-b border-white/10">
      <div class="flex items-center gap-3.5">
        ...
        <div>
          ...
          <h3 class="font-serif text-xl font-normal text-white mt-0.5">Create Discount Coupon</h3>
        </div>
      </div>
    ...
    <div class="p-6 space-y-4 text-xs">
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <!-- Max Usage -->
        <div>
          <label class="block mb-1.5 font-bold uppercase text-[10px] text-gray-500">Redemption Limit (Usage Count)</label>
          <div class="relative">
      ...
      <div class="p-3 bg-[#FAF8F5] rounded-xl border border-[#9B7846]/20 flex items-center gap-2.5 text-[11px] text-gray-600">
        <i class="fa fa-info-circle text-[#9B7846]"></i>
        <span>The unique promotional voucher code will be automatically generated upon creation.</span>
      </div>
    </div>
    ...
    <div class="p-4 bg-gray-50 border-t border-gray-100 flex items-center justify-end gap-3">
      ...
      >
        ...
        <span>{{ isLoading ? "Creating..." : "Save Coupon" }}</span>
      </button>
    </div>
  </form>
</template>
...
async function handleSubmit() {
  ...
  addCoupon(formData)
    ...
    .catch((err) => {
      isLoading.value = false;
      toast.error(err?.response?.data?.errors || "Failed to create coupon");
    });
}
</script>
