  ...
  <form @submit.prevent="handleSubmit" class="text-left font-sans flex flex-col">
    ...
    <div class="p-6 bg-[#132212] text-white flex items-start justify-between border-b border-white/10">
      <div class="flex items-center gap-3.5">
        ...
        <div>
          <span class="text-[10px] uppercase tracking-widest text-[#c5a880] font-bold block">Catalogue Management</span>
          <h3 class="font-serif text-xl font-normal text-white mt-0.5">Add New Suite / Room</h3>
        </div>
      </div>
    ...
    <div class="p-4 bg-gray-50 border-t border-gray-100 flex items-center justify-end gap-3">
      ...
      >
        ...
        <span>{{ isLoading ? "Creating..." : "Save Suite" }}</span>
      </button>
    </div>
  </form>
</template>
...
async function handleSubmit() {
  ...
  addRoom(formData)
    ...
    .catch((err) => {
      isLoading.value = false;
      toast.error(err?.response?.data?.errors || "Error creating room");
    });
}
